...
class _MapScreenState extends State<MapScreen> with TickerProviderStateMixin {
  ...
  void _listenToLocation() {
    final ref = FirebaseDatabase.instance.ref('buses/${widget.busId}');
    ref.onValue.listen((event) {
      final data = event.snapshot.value as Map?;
      if (data != null && mounted) {
        setState(() {
          isActive = data['is_active'] ?? false;
          speed = '${(data['speed'] ?? 0).toStringAsFixed(0)} km/h';
          eta = '~${data['eta'] ?? 8} mins';
        });
      }
    });
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    _infoCard(Icons.speed_rounded, speed, 'Speed', Colors.blue),
                    const SizedBox(width: 12),
                    _infoCard(Icons.location_on_outlined, distance, 'Distance', Colors.orange),
                  ],
                ),
              ),
